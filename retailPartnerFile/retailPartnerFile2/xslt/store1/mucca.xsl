	...
	<xsl:template match="/">
		<produtos>
			...
			<PRODUTO>
				...
				<URL><xsl:value-of select="linkPDP"/>?utm_source=Mucca&amp;utm_medium=xml&amp;utm_campaign=Mucca-<xsl:value-of select="grupo"/>-<xsl:value-of select="categoria"/>-<xsl:value-of select="marca"/>-<xsl:value-of select="codModelo"/>-<xsl:value-of select="codCor"/></URL>
				<IMAGEM><xsl:value-of select="linkImagem"/></IMAGEM>
				<DISPONIBILIDADE>1</DISPONIBILIDADE>
				<VARIAVEIS>
					<VARIAVEL_A><xsl:value-of select="marca"/></VARIAVEL_A>

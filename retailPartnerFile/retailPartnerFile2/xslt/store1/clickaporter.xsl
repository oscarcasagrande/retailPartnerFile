...
<xsl:output method='xml' indent='yes' version='1.0' encoding='UTF-8' standalone='yes' cdata-section-elements='NOME'/> 
	<xsl:template match="/">
		<produtos>
			<xsl:for-each select="catalogo/produto[genero='FEMININO']">
			<PRODUTO>
				...
				<PRECO>
					<DE><xsl:value-of select="precoBase"/></DE>
				...
				<URL><xsl:value-of select="linkPDP"/>?utm_source=clickaporter&amp;utm_medium=xml&amp;utm_campaign=clickaporter-<xsl:value-of select="grupo"/>-<xsl:value-of select="categoria"/>-<xsl:value-of select="marca"/>-<xsl:value-of select="codModelo"/>-<xsl:value-of select="codCor"/></URL>
				<IMAGEM><xsl:value-of select="linkImagem"/></IMAGEM>
				<DISPONIBILIDADE></DISPONIBILIDADE>
				<VARIAVEIS>
					<VARIAVEL_A></VARIAVEL_A>
					<VARIAVEL_B><xsl:value-of select="cor"/></VARIAVEL_B>

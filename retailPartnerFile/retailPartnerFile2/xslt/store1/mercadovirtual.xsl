	...
	<xsl:template match="/">
		<produtos>
			...
			<PRODUTO>
				...
				<URL><xsl:value-of select="linkPDP"/>?utm_source=MercadoVirtual&amp;utm_medium=xml&amp;utm_campaign=MercadoVirtual-<xsl:value-of select="grupo"/>-<xsl:value-of select="categoria"/>-<xsl:value-of select="marca"/>-<xsl:value-of select="codModelo"/>-<xsl:value-of select="codCor"/>
                </URL>

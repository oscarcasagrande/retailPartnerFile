	...
	<xsl:template match="/">
		<centauro>
			...
			<product>
				...
				<link><xsl:value-of select="link"/>?utm_source=Zura&amp;utm_medium=xml&amp;utm_campaign=Zura-<xsl:value-of select="categoria"/>-<xsl:value-of select="subgroup"/>-<xsl:value-of select="brand"/>-<xsl:value-of select="id"/>-<xsl:value-of select="color_code"/>
                </link>

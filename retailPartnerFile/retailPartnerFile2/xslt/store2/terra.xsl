	...
	<xsl:template match="/">
		<produtos>
			...
			<PRODUTO>
				...
				<URL><xsl:value-of select="link"/>?utm_source=Terra&amp;utm_medium=xml&amp;utm_campaign=Terra-<xsl:value-of select="categoria"/>-<xsl:value-of select="subgroup"/>-<xsl:value-of select="brand"/>-<xsl:value-of select="id"/>-<xsl:value-of select="color_code"/>
                </URL>

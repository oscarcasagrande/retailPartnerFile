    ...
    <produtos>
      <xsl:for-each select="rss/channel/item">
        <xsl:if test="saleprice > 0">
          <PRODUTO>
            <IDPRODUTO><xsl:value-of select="substring(sku_id, 1, 8)" /></IDPRODUTO>
            <SKU><xsl:value-of select="sku_id" /></SKU>
            <NOME><xsl:value-of select="title"/></NOME>
            <TAMANHO><xsl:value-of select="size"/></TAMANHO>
            ...
            <URL>http://www.centauro.com.br/promotion2.jsp;$urlparam$811mw2QFyKi2JljHqwxfpel1WOpCWA?page=pdp&amp;productId=<xsl:value-of select="id"/>&amp;utm_source=Vtex&amp;utm_medium=xml&amp;utm_campaign=Vtex-<xsl:value-of select="categoria"/>-<xsl:value-of select="subgroup"/>-<xsl:value-of select="brand"/>-<xsl:value-of select="id"/>-<xsl:value-of select="color_code"/></URL>
            <IMAGEM><xsl:value-of select="image_link"/></IMAGEM>
            <DISPONIBILIDADE>1</DISPONIBILIDADE>
            <VARIAVEIS>
              <VARIAVEL_A></VARIAVEL_A>
              <VARIAVEL_B><xsl:value-of select="color_code"/>?utm_source=Vtex&amp;utm_medium=xml&amp;utm_campaign=Vtex-<xsl:value-of select="categoria"/>-<xsl:value-of select="subgroup"/>-<xsl:value-of select="brand"/>-<xsl:value-of select="id"/>-<xsl:value-of select="color_code"/></VARIAVEL_B>

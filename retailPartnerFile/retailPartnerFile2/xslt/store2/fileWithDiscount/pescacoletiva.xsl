	...
	<xsl:template match="/">
      ...
      <xsl:for-each select="rss/channel/item[generate-id() = generate-id(key('distinctModeloCor', substring(sku_id, 1, 8)))]">
			<PRODUTO>
				...
				<URL><xsl:value-of select="link"/>?utm_source=pescacoletiva&amp;utm_medium=xml&amp;utm_campaign=pescacoletiva-<xsl:value-of select="categoria"/>-<xsl:value-of select="subgroup"/>-<xsl:value-of select="brand"/>-<xsl:value-of select="id"/>-<xsl:value-of select="color_code"/></URL>

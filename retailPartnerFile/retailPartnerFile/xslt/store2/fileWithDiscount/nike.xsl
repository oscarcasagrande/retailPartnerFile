      ...
      <xsl:for-each select="rss/channel/item[brand_code='0073']">
        <xsl:if test="saleprice > 0 and numberOfInstallment > 0 and amountPerInstallment > 0">
          <PRODUTO>
            ...
            <URL>
              http://www.centauro.com.br/promotion2.jsp;$urlparam$811mw2QFyKi2JljHqwxfpel1WOpCWA?page=pdp&amp;productId=<xsl:value-of select="id"/>&amp;utm_source=Nike&amp;utm_medium=xml&amp;utm_campaign=Nike-<xsl:value-of select="categoria"/>-<xsl:value-of select="subgroup"/>-<xsl:value-of select="brand"/>-<xsl:value-of select="id"/>-<xsl:value-of select="color_code"/>
            </URL>
            <IMAGEM>
              http://images.centauro.com.br/900x900/<xsl:value-of select="id"/><xsl:value-of select="color_code"/>.jpg
            </IMAGEM>
            <TAMANHO>

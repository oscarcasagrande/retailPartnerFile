  ...
  <xsl:template match="/">
    <produtos>
      <xsl:for-each select="rss/channel/item">
        <PRODUTO>
          <SKU><xsl:value-of select="id"/><xsl:value-of select="color_code"/>
          </SKU>
          ...
          <VARIAVEIS>
            ...
            <VARIAVEL_B><xsl:value-of select="color_code"/>?utm_source=TudoCupom&amp;utm_medium=xml&amp;utm_campaign=TudoCupom-<xsl:value-of select="categoria"/>-<xsl:value-of select="subgroup"/>-<xsl:value-of select="brand"/>-<xsl:value-of select="id"/>-<xsl:value-of select="color_code"/></VARIAVEL_B>

    ...
    <channel>
      <xsl:for-each select="rss/channel/item">
        <Produto>
          ...
          <LinkSite>
            <xsl:value-of select="link"/>?utm_source=Iconna&amp;utm_medium=xml&amp;utm_campaign=Iconna-<xsl:value-of select="categoria"/>-<xsl:value-of select="subgroup"/>-<xsl:value-of select="brand"/>-<xsl:value-of select="id"/>-<xsl:value-of select="color_code"/>
          </LinkSite>
          <LinkImagem>

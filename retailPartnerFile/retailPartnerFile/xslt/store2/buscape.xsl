      ...
      <xsl:for-each select="rss/channel/item[sku_id!='769822000000']">
        <PRODUTO>
          ...
          <VARIAVEIS>
            ...
            <VARIAVEL_B>
              <xsl:value-of select="color_code"/>?utm_source=Buscape&amp;utm_medium=xml&amp;utm_campaign=Buscape-<xsl:value-of select="categoria"/>-<xsl:value-of select="subgroup"/>-<xsl:value-of select="brand"/>-<xsl:value-of select="id"/>-<xsl:value-of select="color_code"/>
            </VARIAVEL_B>

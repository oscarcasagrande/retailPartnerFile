    ...
    <produtos>
      <xsl:for-each select="catalogo/produto[marca='Nike']">
        <PRODUTO>
          ...
          <PRECOPOR>
            <xsl:value-of select="format-number(precoPromocional, '###.###,00', 'real')"/>
          </PRECOPOR>
          <N_PARCELAS>
          ...
          <URL>
            <xsl:value-of select="linkPDP"/>?utm_source=Nike&amp;utm_medium=xml&amp;utm_campaign=Nike-<xsl:value-of select="grupo"/>-<xsl:value-of select="categoria"/>-<xsl:value-of select="marca"/>-<xsl:value-of select="codModelo"/>-<xsl:value-of select="codCor"/>
          </URL>
          <IMAGEM>

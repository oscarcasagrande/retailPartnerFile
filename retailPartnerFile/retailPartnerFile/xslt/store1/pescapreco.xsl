	...
	<xsl:template match="/">
		<lojista>
			...
			<produto>
				 ...
				 <link_prod><xsl:value-of select="linkPDP"/>?utm_source=PescaPreco&amp;utm_medium=xml&amp;utm_campaign=PescaPreco-<xsl:value-of select="grupo"/>-<xsl:value-of select="categoria"/>-<xsl:value-of select="marca"/>-<xsl:value-of select="codModelo"/>-<xsl:value-of select="codCor"/>
                 </link_prod>
				 <imagem><xsl:value-of select="linkImagem"/></imagem>
				 <categ><xsl:value-of select="grupo"/>: <xsl:value-of select="categoria"/></categ>

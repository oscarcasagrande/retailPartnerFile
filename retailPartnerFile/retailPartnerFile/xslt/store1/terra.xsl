    ...
    <produtos>
      <xsl:for-each select="catalogo/produto">
        <PRODUTO>
          ...
          <URL>
            <xsl:value-of select="linkPDP"/>?utm_source=Terra&amp;utm_medium=xml&amp;utm_campaign=Terra-<xsl:value-of select="grupo"/>-<xsl:value-of select="categoria"/>-<xsl:value-of select="marca"/>-<xsl:value-of select="codModelo"/>-<xsl:value-of select="codCor"/>
          </URL>
          <IMAGEM>

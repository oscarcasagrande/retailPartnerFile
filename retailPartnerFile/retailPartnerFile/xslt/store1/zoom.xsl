    ...
    <lojista>
      <xsl:for-each select="catalogo/produto">
        <produto>
          ...
          <link_prod>
            <xsl:value-of select="linkPDP"/>?utm_source=Zoom&amp;utm_medium=xml&amp;utm_campaign=Zoom-<xsl:value-of select="grupo"/>-<xsl:value-of select="categoria"/>-<xsl:value-of select="marca"/>-<xsl:value-of select="codModelo"/>-<xsl:value-of select="codCor"/>
          </link_prod>
          <imagem>

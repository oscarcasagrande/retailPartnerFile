      ...
      <xsl:for-each select="catalogo/produto">
        <PRODUTO>
          ...
          <VARIAVEIS>
            ...
            <VARIAVEL_B>
              <xsl:value-of select="codCor"/>?utm_source=TudoCupom&amp;utm_medium=xml&amp;utm_campaign=TudoCupom-<xsl:value-of select="grupo"/>-<xsl:value-of select="categoria"/>-<xsl:value-of select="marca"/>-<xsl:value-of select="codModelo"/>-<xsl:value-of select="codCor"/>
            </VARIAVEL_B>
